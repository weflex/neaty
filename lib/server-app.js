...
proto.start = function() {
  var app = this;
  app.http = require('http').Server(app).listen(
    app.get('port'),
    app.get('host'),
    function() {
      app.emit('started');
  ...
  var authConfig = app.get('authorization');
  if (authConfig && authConfig.enabled) {
    app.enableAuth(authConfig.options);
  }
  var websocketConfig = app.get('websocket');
  if (websocketConfig && websocketConfig.enabled) {
    app.io = require('socket.io')(app.http);
    app.io.on('connection', onconnect);
    app.emit('websocket.ready', app.io);

    if (websocketConfig.authorization) {
      const socketioAuth = require('socketio-auth');
      websocketConfig.authorization.forEach((name) => {
        socketioAuth(app.io.of(name || '/'), {
    ...
    function onconnect(socket) {
      app.emit('websocket.connection', socket);
      app.on('disconnect', ondisconnect);
    }
    function ondisconnect() {
      app.emit('websocket.disconnect');
    }
    function authenticate(socket, credentials, next) {
      var accessTokenModel = app.models.AccessToken;
      try {
        accessTokenModel = app.models[authConfig.options.models.AccessToken];
      } catch (err) {}
      accessTokenModel.findOne({where: credentials}, function(err, token) {
        if (!err || token) next(null, token);
        else next(err, false);
      });
    }
  }
...
proto.middlewareFromConfig = function(factory, config) {
  ...
  var handler = factory.apply(this, params);
