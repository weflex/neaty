...
module.exports = function(registry) {
  ...
  Model.setup = function() {
    ...
    // create validations
    var validations = sharedClass.ctor.settings.validations || [];
    validations.forEach(function(validation) {
      var valid = validation.name && sharedClass.ctor[validation.name];
      if (typeof valid === 'function') {
        if (validation.property) {
          validation.args = [validation.property].concat(validation.args);
          delete validation.property;
        }
        valid.apply(sharedClass.ctor, validation.args);
      }
    });
  ...
  Model.checkAccess = function(token, modelId, sharedMethod, ctx, callback) {
    ...
    aclModel.checkAccessForContext({
      ...
      remotingContext: ctx
    }, function(err, accessRequest, propertyACLs) {
      if (err) return callback(err);
      callback(null, accessRequest.isAllowed(), propertyACLs);
    });
  };
